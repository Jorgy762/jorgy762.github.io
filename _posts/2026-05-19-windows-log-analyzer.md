...
date: 2026-05-19
tags: [tools, windows, security, sysadmin, blue-team, log-analysis]
excerpt: "A browser-based tool for parsing Windows Event XML exports, IIS W3C logs, and plain text logs. Filter by severity, search, visualize, and export -- no installs, no data leaves your browser."
---
...
This tool is what I built to fix that. Drop in a log file and it parses, filters, charts, and exports. Everything runs client-side. No server, no upload, no third-party anything seeing your data.
...
| Format | Source |
|---|---|
| Windows Event XML | Event Viewer > right-click any log > **Save All Events As...** > XML |
| IIS W3C Log | `C:\inetpub\logs\LogFiles\W3SVC1\u_ex*.log` |
| Plain Text | Any `.log` or `.txt` file with timestamped lines |

> **On .evtx files:** Binary EVTX is not parseable in a browser. Export from Event Viewer first using the steps above, or convert on the command line with PowerShell:
> ```powershell
...
(function(){
  ...
  function parseEventXML(content) {
    ...
    xmlDoc.querySelectorAll('Event').forEach(ev => {
      ...
      const rawTime = ((sys.querySelector('TimeCreated') || {}).getAttribute || (() => ''))('SystemTime') || '';
      let timeStr = rawTime;
      ...
      const rendered = (ev.querySelector('RenderingInfo > Message') || {}).textContent || '';

      records.push({
        time:    timeStr,
        eventId: (sys.querySelector('EventID') || {}).textContent || '',
        level,
        source:  ((sys.querySelector('Provider') || {}).getAttribute || (() => ''))('Name') || '',
  ...
  function applyFilters() {
    ...
    if (filtered.length > MAX_ROWS) {
      fs.textContent = 'Showing first ' + MAX_ROWS.toLocaleString() + ' matches -- narrow your filters to see more.';
    } else {
      fs.textContent = '';
    }
  }
...
The tool caps table rendering at 2,500 rows at a time for performance. If you're working with a high-volume log and the filter status line tells you it hit the limit, narrow your search or level filter and the full filtered set still exports to CSV or JSON intact.

If you run into a format this doesn't handle -- Application event channels, Windows Firewall logs, DHCP audit logs -- drop a comment and I'll add a parser for it.
